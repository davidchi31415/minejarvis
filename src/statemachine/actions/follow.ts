...
function createFollowPlayerActionState(bot, data) {
  ...
  const getClosestPlayerState = new BehaviorGetClosestEntity(
    bot,
    targets,
  ...
  const transitions = [
    ...
    new StateTransition({
      ...
      shouldTransition: () =>
        followPlayerState.distanceToTarget() < followRadius,
    }),
    ...
    new StateTransition({
      ...
      child: followPlayerState,
      shouldTransition: () =>
        lookAtPlayerState.distanceToTarget() >= followRadius,
    }),
  ];
